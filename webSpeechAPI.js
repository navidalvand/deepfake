// ------- Cross-browser Speech-to-Text (Web Speech API) ------- //

// Cache DOM elements (self-contained: avoids reliance on variables from main.js)
...
// Prefer the standard API, fall back to the WebKit prefix
const SpeechRecognitionAPI =
  window.SpeechRecognition || window.webkitSpeechRecognition;

if (!SpeechRecognitionAPI) {
  // Speech recognition not supported – disable the mic button (if present) and warn the developer
  console.warn(
    "SpeechRecognition/Web Speech API is not supported in this browser."
  );
  if (typeof speechButton !== "undefined") {
    speechButton.setAttribute("disabled", true);
    speechButton.title = "Speech-to-text is not supported in this browser";
  }
  // In unsupported environments, we leave `recognition` undefined and simply skip
  // attaching handlers below. The speech button is disabled, so users won’t be
  // able to trigger the feature.
} else {
  // Create the recogniser instance
  recognition = new SpeechRecognitionAPI();

  // Set recognition language (BCP-47 language tag)
...
function toggleStartStop() {
  // If SpeechRecognition is not available, simply exit.
